...
export class OrdersAPI {
  ...
  static async getOrderAnalytics(
    ...
    type OrderData = { status: string; total_amount: number; created_at: string };
    
    const analytics = {
      totalOrders: data?.length || 0,
      totalRevenue: data?.reduce((sum: number, order: OrderData) => sum + order.total_amount, 0) || 0,
      ordersByStatus: data?.reduce((acc: Record<string, number>, order: OrderData) => {
        acc[order.status] = (acc[order.status] || 0) + 1;
        return acc;
      }, {} as Record<string, number>) || {},
      averageOrderValue: data?.length ? 
        (data.reduce((sum: number, order: OrderData) => sum + order.total_amount, 0) / data.length) : 0,
    };
