...
export class SellerDashboardAPI {
  ...
  static async getRevenueTrends(
    storeId: string,
    // Period parameter is currently unused but kept for future implementation
  ...
  static async getTopProducts(
    ...
    // Use select with group by instead of the .group() method
    const { data, error } = await query
      .select('product_id, products.title, products.name, products.price, products.store_id, count(*) as total_quantity, sum(total_price) as total_revenue')
  ...
  static async getCustomerInsights(storeId: string) {
    ...
    orders?.forEach((order: {user_id: string; total_amount: number; created_at: string}) => {
